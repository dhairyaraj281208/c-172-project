...
AFRAME.registerComponent("markerhandler", {
  ...
  handleMarkerFound: function (toys, markerId) {
    ...
    if (toy.is_out_of_stock === true) {
      ...
      orderButtton.addEventListener("click", () => {
        var tNumber;
        userNumber <= 9 ? (tNumber = `U0${userNumber}`) : `U${userNumber}`;
  ...
  handleOrderSummary: async function () {
    ...
    //Getting user Number
    var tNumber;
    userNumber <= 9 ? (tNumber = `T0${userNumber}`) : `T${userNumber}`;
    ...
    currentOrders.map(i => {
      ...
      //Create user cells/columns for ITEM NAME, PRICE, QUANTITY & TOTAL PRICE
      ...
      //Append row to the user
      userBodyTag.appendChild(tr);
    });

    //Create a user row to Total bill
